...
from ...types.keywords import movie_list_params
from ...types.keywords.movie_list_response import MovieListResponse
...
class MoviesResource(SyncAPIResource):
    ...
    def list(
        ...
        *,
        ...
        """
        if not keyword_id:
            raise ValueError(f"Expected a non-empty value for `keyword_id` but received {keyword_id!r}")
        return self._get(
            f"/3/keyword/{keyword_id}/movies",
