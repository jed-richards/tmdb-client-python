...
class TestTvEpisodes:
    ...
    def test_method_list(self, client: TmdbClient) -> None:
        tv_episode = client.accounts.rated.tv_episodes.list(
            account_id=0,
        )
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.accounts.rated.tv_episodes.with_raw_response.list(
            account_id=0,
        )
    ...
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.accounts.rated.tv_episodes.with_streaming_response.list(
            account_id=0,
        ) as response:
...
class TestAsyncTvEpisodes:
    ...
    async def test_method_list(self, async_client: AsyncTmdbClient) -> None:
        tv_episode = await async_client.accounts.rated.tv_episodes.list(
            account_id=0,
        )
        assert_matches_type(TvEpisodeListResponse, tv_episode, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.accounts.rated.tv_episodes.with_raw_response.list(
            account_id=0,
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.accounts.rated.tv_episodes.with_streaming_response.list(
            account_id=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
