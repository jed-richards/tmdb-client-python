...
class TestMovies:
    ...
    def test_method_list(self, client: TmdbClient) -> None:
        movie = client.accounts.rated.movies.list(
            account_id=0,
        )
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.accounts.rated.movies.with_raw_response.list(
            account_id=0,
        )
    ...
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.accounts.rated.movies.with_streaming_response.list(
            account_id=0,
        ) as response:
...
class TestAsyncMovies:
    ...
    async def test_method_list(self, async_client: AsyncTmdbClient) -> None:
        movie = await async_client.accounts.rated.movies.list(
            account_id=0,
        )
        assert_matches_type(MovieListResponse, movie, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.accounts.rated.movies.with_raw_response.list(
            account_id=0,
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.accounts.rated.movies.with_streaming_response.list(
            account_id=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
