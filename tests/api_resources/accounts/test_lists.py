...
class TestLists:
    ...
    def test_method_list(self, client: TmdbClient) -> None:
        list_ = client.accounts.lists.list(
            account_id=0,
        )
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.accounts.lists.with_raw_response.list(
            account_id=0,
        )
    ...
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.accounts.lists.with_streaming_response.list(
            account_id=0,
        ) as response:
...
class TestAsyncLists:
    ...
    async def test_method_list(self, async_client: AsyncTmdbClient) -> None:
        list_ = await async_client.accounts.lists.list(
            account_id=0,
        )
        assert_matches_type(ListListResponse, list_, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.accounts.lists.with_raw_response.list(
            account_id=0,
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.accounts.lists.with_streaming_response.list(
            account_id=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
