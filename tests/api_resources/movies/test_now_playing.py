...
class TestNowPlaying:
    ...
    def test_method_list(self, client: TmdbClient) -> None:
        now_playing = client.movies.now_playing.list()
        assert_matches_type(NowPlayingListResponse, now_playing, path=["response"])
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.movies.now_playing.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        now_playing = response.parse()
        assert_matches_type(NowPlayingListResponse, now_playing, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.movies.now_playing.with_streaming_response.list() as response:
...
class TestAsyncNowPlaying:
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.movies.now_playing.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        now_playing = await response.parse()
        assert_matches_type(NowPlayingListResponse, now_playing, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.movies.now_playing.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
