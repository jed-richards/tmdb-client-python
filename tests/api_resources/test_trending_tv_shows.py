...
class TestTrendingTvShows:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_list(self, client: TmdbClient) -> None:
        trending_tv_show = client.trending_tv_shows.list(
            time_window="day",
        )
        assert_matches_type(TrendingTvShowListResponse, trending_tv_show, path=["response"])
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.trending_tv_shows.with_raw_response.list(
            time_window="day",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        trending_tv_show = response.parse()
        assert_matches_type(TrendingTvShowListResponse, trending_tv_show, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.trending_tv_shows.with_streaming_response.list(
            time_window="day",
        ) as response:
...
class TestAsyncTrendingTvShows:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_list(self, async_client: AsyncTmdbClient) -> None:
        trending_tv_show = await async_client.trending_tv_shows.list(
            time_window="day",
        )
        assert_matches_type(TrendingTvShowListResponse, trending_tv_show, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.trending_tv_shows.with_raw_response.list(
            time_window="day",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        trending_tv_show = await response.parse()
        assert_matches_type(TrendingTvShowListResponse, trending_tv_show, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.trending_tv_shows.with_streaming_response.list(
            time_window="day",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
