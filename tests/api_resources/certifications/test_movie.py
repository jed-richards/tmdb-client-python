...
class TestMovie:
    ...
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.certifications.movie.with_streaming_response.list() as response:
...
class TestAsyncMovie:
    ...
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.certifications.movie.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
