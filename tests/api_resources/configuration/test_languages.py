...
class TestLanguages:
    ...
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.configuration.languages.with_streaming_response.list() as response:
...
class TestAsyncLanguages:
    ...
    async def test_method_list(self, async_client: AsyncTmdbClient) -> None:
        language = await async_client.configuration.languages.list()
        assert_matches_type(LanguageListResponse, language, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.configuration.languages.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.configuration.languages.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
