...
class TestImages:
    ...
    def test_method_list(self, client: TmdbClient) -> None:
        image = client.collections.images.list(
            collection_id=0,
        )
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.collections.images.with_raw_response.list(
            collection_id=0,
        )
    ...
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.collections.images.with_streaming_response.list(
            collection_id=0,
        ) as response:
...
class TestAsyncImages:
    ...
    async def test_method_list(self, async_client: AsyncTmdbClient) -> None:
        image = await async_client.collections.images.list(
            collection_id=0,
        )
        assert_matches_type(ImageListResponse, image, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.collections.images.with_raw_response.list(
            collection_id=0,
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.collections.images.with_streaming_response.list(
            collection_id=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
