...
class TestTopRated:
    ...
    def test_method_list(self, client: TmdbClient) -> None:
        top_rated = client.tv_shows.top_rated.list()
        assert_matches_type(TopRatedListResponse, top_rated, path=["response"])
    ...
    def test_raw_response_list(self, client: TmdbClient) -> None:
        response = client.tv_shows.top_rated.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        top_rated = response.parse()
        assert_matches_type(TopRatedListResponse, top_rated, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: TmdbClient) -> None:
        with client.tv_shows.top_rated.with_streaming_response.list() as response:
...
class TestAsyncTopRated:
    ...
    async def test_raw_response_list(self, async_client: AsyncTmdbClient) -> None:
        response = await async_client.tv_shows.top_rated.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        top_rated = await response.parse()
        assert_matches_type(TopRatedListResponse, top_rated, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncTmdbClient) -> None:
        async with async_client.tv_shows.top_rated.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
